...
class Lotka_Task():
    ...
    def eval_post_log_prob_given_samples(self, samples, X_dict):
        """
        Given the samples size: (N, 8)
        Evaluate the posterior score function at those samples
        """
        ...
        score_unconstrainedsamples = torch.zeros(samples.shape[0], 8)
        ...
        for i in range(samples.shape[0]):
            ...
            #print("Samples params: ", dict(theta=theta[i,:].tolist(), \
            #                                                               z_init=z_init[i,:].tolist(), \
            #                                                               sigma=sigma[i,:].tolist()))

            print("samples: ", samples[i,:].tolist())
            temp = posterior.grad_log_prob(samples[i,:].tolist())
    ...
    def post_inference(self, X_dict, nchains, nsams_perchain):
        ...
        for i in range(nchains * nsams_perchain):
            ...
            #print("Posterior params: ", dict(theta=theta[:, i].tolist(), \
            #                                                               z_init=z_init[:, i].tolist(), \
            #                                                               sigma=sigma[:, i].tolist()))
            
            #print("Posterior unconstrained param: ", posterior.unconstrain_pars(dict(theta=theta[:, i].tolist(), \
            #                                                               z_init=z_init[:, i].tolist(), \
            #                                                               sigma=sigma[:, i].tolist())))
...
def get_all_data(lynx_hare_data, num_chains_true_intval, num_samperchain_true_intval):

    task_dist = Lotka_Task_Distribution(lynx_hare_data)
...
def get_score_on_prior_samples(lynx_hare_data):
    prior_samples = torch.load('prior_samples_unc.pt')

    
    task_dist = Lotka_Task(lynx_hare_data)
    data_dict = task_dist.sample_observations(size = lynx_hare_data['N'])

    # evaluate score function on 
    score_unc_prior = task_dist.eval_post_log_prob_given_samples(prior_samples, data_dict)

    # save
    torch.save(score_unc_prior, 'score_unc_prior.pt')

    return score_unc_prior 
...
### save score function for prior samples
get_score_on_prior_samples(lynx_hare_data)
exit()

###### saving data
# get all data

#all_data = get_all_data(lynx_hare_data=lynx_hare_data, num_chains_true_intval=4, num_samperchain_true_intval=1000)
#torch.save(all_data, 'all_data_lv.pt')

#print("Saved all_data.pt!")
