...
"""
Kernel classes (for use with Control Functional)
"""
...
class rbf_kernel(object):
   ...
   #       --- The kernel_parm2 is l^2 (to match the def. of median heuristic of l^2), the postivity of it are assured by exp transformation in the process of tuning them
    def __init__(self):
        self._kernel_parm1 = torch.ones(1).to(device) # convert to torch
        self._kernel_parm2 = torch.ones(1).to(device)
    ...
    def deriv_base_kernel(self, x, y):
        ...
        self.kernel_parm1 = self.kernel_parm1.to(device)
        self.kernel_parm2 = self.kernel_parm2.to(device)
