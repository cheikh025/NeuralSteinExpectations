...
class negative_log_marginal_lik_MRI_singledat(torch.nn.Module):
    ...
    def forward(self, batch_sample_indices, beta_cstkernel = 1):
        ...
        k_XbXb.to(X_batch.device)

        cond_number_threshold = 1e6
        lam = 1e-6
        bad_cond = np.linalg.cond(k_XbXb.detach().cpu().numpy()) >= cond_number_threshold
        k_Yb = k_XbXb + lam * torch.eye(X_batch.size()[0]).to(X_batch.device)
        while bad_cond:
            lam = 10 * lam
            k_Yb = k_XbXb + lam * torch.eye(X_batch.size()[0]).to(X_batch.device)
            bad_cond = np.linalg.cond(k_Yb.detach().cpu().numpy()) >= cond_number_threshold
